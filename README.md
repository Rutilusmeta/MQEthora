# MetaQueer Superapp forked from `Ethora`

## Prerequisites

Since we are going to focus on `client-web` to build **MQ Superapp**:
...
Prepare `.env` from `.env.backup` file:
...
_Note: Some information might be outdated - we try to update documentation here and in the website as often as possible, but if you're not sure please feel free to contact us._
