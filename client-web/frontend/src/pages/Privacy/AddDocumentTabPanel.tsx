...
interface AddDocumentTabPanelProperties {
  getSharedLinks: () => Promise<void>
}

interface optionsItemProperties {
...
const documentDescription = "Choose the Document you would like to share."

export const AddDocumentTabPanel = (
  properties: AddDocumentTabPanelProperties
) => {
...
function OptionsItem(properties: optionsItemProperties) {
  const { handleChange, selectedValue, title, description, type, documents } =
    properties
    ...
    >
      ...
      {type === "menu1" ? (
        <FormControl sx={{ minWidth: 120, marginTop: "20px" }}>
          ...
          >
            <MenuItem value={(-1).toString()}>No Expiration</MenuItem>
            <MenuItem value={HOUR.toString()}>1 hour</MenuItem>
            <MenuItem value={DAY.toString()}>1 day</MenuItem>
            <MenuItem value={WEEK.toString()}>1 week</MenuItem>
            <MenuItem value={MONTH.toString()}>1 month</MenuItem>
          </Select>
        </FormControl>
      ) : null}
      {type === "menu2" ? (
        <FormControl sx={{ minWidth: 120, marginTop: "20px" }}>
          ...
          >
            {documents.map((item) => (
              <MenuItem value={item._id}>{item.documentName}</MenuItem>
            ))}
          </Select>
        </FormControl>
      ) : null}
